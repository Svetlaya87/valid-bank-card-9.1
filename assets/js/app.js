	...
	let re = /[0-9]{16,16}|[0-9]{4,4}\-{1,1}[0-9]{4,4}\-{1,1}[0-9]{4,4}\-{1,1}[0-9]{4,4}|[0-9]{4,4}\s{1,1}[0-9]{4,4}\s{1,1}[0-9]{4,4}\s{1,1}[0-9]{4,4}/g;

	// /(^[0-9]{16}$)|(^[0-9]{4}\-[0-9]{4}\-[0-9]{4}\-[0-9]{4}$)|(^[0-9]{4}\s[0-9]{4}\s[0-9]{4}\s[0-9]{4}$)/g; со скобками при глобальном поиске чет не работает, но я не поняла почему
	// \s пробел, \- экранированное тире
	...
	for(let j=0; j<chek.length;j++) {		
		for(let i=0; i<chek[j].length; i++){
			chek[j][i]=chek[j][i]*1; // преобразование каждого символва к числу
		}
	};
